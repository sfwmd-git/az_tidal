...
# forecast GRIB Dims.
forecast_nlat_idx = station_data['ecmwf_det_forecast_nlat_idx']
forecast_slat_idx = station_data['ecmwf_det_forecast_slat_idx']
forecast_wlon_idx = station_data['ecmwf_det_forecast_wlon_idx']
forecast_elon_idx = station_data['ecmwf_det_forecast_elon_idx']

# wave GRIB Dims.
wave_nlat_idx = station_data['ecmwf_det_wave_nlat_idx']
wave_slat_idx = station_data['ecmwf_det_wave_slat_idx']
wave_wlon_idx = station_data['ecmwf_det_wave_wlon_idx']
wave_elon_idx = station_data['ecmwf_det_wave_elon_idx']
...
def get_average_slp(file):
    with pygrib.open(file) as gr:
        for msg in gr:
            if msg.name == 'Mean sea level pressure':
                vals = msg.values
...
def get_average_sst(file):
    with pygrib.open(file) as gr:
        for msg in gr:
            if msg.name == 'Sea surface temperature':
                vals = msg.values
                avg = np.mean(vals[forecast_nlat_idx:forecast_slat_idx+1, forecast_wlon_idx:forecast_elon_idx+1])  # Calculate the average over the specified slice
                return avg
    return None
...
def get_average_u10(file):
    with pygrib.open(file) as gr:
        for msg in gr:
            if msg.name == '10 metre U wind component':
                vals = msg.values
                avg = np.mean(vals[forecast_nlat_idx:forecast_slat_idx+1, forecast_wlon_idx:forecast_elon_idx+1])  # Calculate the average over the specified slice
                return avg
    return None
...
def get_average_v10(file):
    with pygrib.open(file) as gr:
        for msg in gr:
            if msg.name == '10 metre V wind component':
                vals = msg.values
                avg = np.mean(vals[forecast_nlat_idx:forecast_slat_idx+1, forecast_wlon_idx:forecast_elon_idx+1])  # Calculate the average over the specified slice
                return avg
    return None
...
def get_average_swh(file):
    with pygrib.open(file) as gr:
        for msg in gr:
            if msg.name == 'Significant height of combined wind waves and swell':
                vals = msg.values
...
def generate_forecast_file_name(creation_date, forecast_hour_difference):
    ...
    if creation_date.strftime("%m%d%H") == target_date.strftime("%m%d%H"):
        ...
        formatted_target_date = target_date.strftime("%m%d%H") + "01"
    else:
    ...
    return f"A1D{formatted_creation_date}{formatted_target_date}1"


def process_forecast_grib_files(db):
    data = []
...
def generate_wave_file_name(creation_date, forecast_hour_difference):
    ...
    if creation_date.strftime("%m%d%H") == target_date.strftime("%m%d%H"):
        ...
        formatted_target_date = target_date.strftime("%m%d%H%M")
    # Combine the formatted dates according to the specified structure
    return f"A1P{formatted_creation_date}{formatted_target_date}1"


def process_wave_grib_files(db):
    data = []
...
#------------------------------------------------------------------------------
#                 GET AND CALC EURO FORECASTS
#------------------------------------------------------------------------------
os.chdir(pwd)
euro = process_forecast_grib_files(db)
# print(euro)
euro['datetime_est'] = pd.to_datetime(euro['datetime_est'])
# Remove timezone information and set the index to 'datetime_est'
euro.index = euro['datetime_est']
euro = euro.drop(columns=['datetime_est'])
# Calculate daily average for 'euro'
euro = euro.resample('D').mean()

#------------------------------------------------------------------------------
#                 READ AND CALC SWH 
#------------------------------------------------------------------------------
swh = process_wave_grib_files(db)
print(swh)
...
euro_reset = euro.reset_index().rename(columns={'datetime_est': 'date'})
euro_reset['date'] = euro_reset['date'].dt.strftime('%Y-%m-%d')

# Merge the DataFrames on the 'date' column
forecast = pd.merge(euro_reset, swh_reset, on='date')
...
predictions.to_csv(f"{OUTPUT_DIR}{today.strftime('%Y-%m-%d')}_{station_abbr}_{model}_{forecast_type}_MHHW_all_data.csv", index=False)
...
components.to_csv(f"{OUTPUT_DIR}{today.strftime('%Y-%m-%d')}_{station_abbr}_{model}_{forecast_type}_MHHW_components.csv", index=False)
...
pred.to_csv(f"{OUTPUT_DIR}{today.strftime('%Y-%m-%d')}_{station_abbr}_{model}_{forecast_type}_MHHW_predictions_daily.csv", index=False)
...
# Calculate the effects
predictions_NAVD["slp_effect"] = predictions_NAVD["slp_delta"] * -0.0328084
...
# Calculate my_prediction
predictions_NAVD["station_SLP_prediction"] = (
...
predictions_NAVD.to_csv(f"{OUTPUT_DIR}{today.strftime('%Y-%m-%d')}_{station_abbr}_{model}_{forecast_type}_NAVD_all_data.csv", index=False)
...
components_NAVD.to_csv(f"{OUTPUT_DIR}{today.strftime('%Y-%m-%d')}_{station_abbr}_{model}_{forecast_type}_NAVD_components.csv", index=False)
...
pred_NAVD.to_csv(f"{OUTPUT_DIR}{today.strftime('%Y-%m-%d')}_{station_abbr}_{model}_{forecast_type}_NAVD_predictions_daily.csv", index=False)
...
merged_df["noaa_slp_offset"] = merged_df['station_SLP_prediction'] - merged_df['noaa']
merged_df["noaa_full_offset"] = merged_df['station_prediction'] - merged_df['noaa']
merged_df[['noaa_slr_offset', 'noaa_slp_offset', 'noaa_full_offset']] = merged_df[['noaa_slr_offset', 'noaa_slp_offset', 'noaa_full_offset']].interpolate(method='linear')
merged_df[['noaa_slr_offset', 'noaa_slp_offset', 'noaa_full_offset']] = merged_df[['noaa_slr_offset', 'noaa_slp_offset', 'noaa_full_offset']].fillna(method='bfill')

# Convert original columns to interpolated values
merged_df['noaa_slr'] = merged_df['noaa'] + merged_df['noaa_slr_offset']
merged_df['station_SLP_prediction'] = merged_df['noaa'] + merged_df['noaa_slp_offset']
merged_df['station_prediction'] = merged_df['noaa'] + merged_df['noaa_full_offset']

# drop columns
merged_df = merged_df.drop(columns=['days_ahead', 'noaa_slr_offset','noaa_slp_offset','noaa_full_offset','date'])
...
plot_data.loc[:239, 'station_SLP_prediction'] = np.nan

# Output to csv
plot_data.to_csv(f"{OUTPUT_DIR}{today.strftime('%Y-%m-%d')}_{station_abbr}_{model}_{forecast_type}_MHHW_predictions_6min.csv", index=False)
...
pred_NAVD.at[0, 'station_prediction'] = predictions_NAVD['Observation'].iloc[0]
...
merged_df_NAVD["noaa_slp_offset"] = merged_df_NAVD['station_SLP_prediction'] - merged_df_NAVD['noaa']
merged_df_NAVD["noaa_full_offset"] = merged_df_NAVD['station_prediction'] - merged_df_NAVD['noaa']
merged_df_NAVD[['noaa_slr_offset', 'noaa_slp_offset', 'noaa_full_offset']] = merged_df_NAVD[['noaa_slr_offset', 'noaa_slp_offset', 'noaa_full_offset']].interpolate(method='linear')
merged_df_NAVD[['noaa_slr_offset', 'noaa_slp_offset', 'noaa_full_offset']] = merged_df_NAVD[['noaa_slr_offset', 'noaa_slp_offset', 'noaa_full_offset']].fillna(method='bfill')

# Convert original columns to interpolated values
merged_df_NAVD['noaa_slr'] = merged_df_NAVD['noaa'] + merged_df_NAVD['noaa_slr_offset']
merged_df_NAVD['station_SLP_prediction'] = merged_df_NAVD['noaa'] + merged_df_NAVD['noaa_slp_offset']
merged_df_NAVD['station_prediction'] = merged_df_NAVD['noaa'] + merged_df_NAVD['noaa_full_offset']

# drop columns
merged_df_NAVD = merged_df_NAVD.drop(columns=['days_ahead', 'noaa_slr_offset','noaa_slp_offset','noaa_full_offset','date'])
...
plot_data_NAVD.loc[:239, 'station_SLP_prediction'] = np.nan

# Output to csv
plot_data_NAVD.to_csv(f"{OUTPUT_DIR}{today.strftime('%Y-%m-%d')}_{station_abbr}_{model}_{forecast_type}_NAVD_predictions_6min.csv", index=False)
...
ax.plot(components['date'], components['station_SLP_prediction'], color='g', label="NOAA + SLR + SLP", linewidth=2)
ax.plot(components['date'], components['station_prediction'], color='cyan', label="Model Forecast", linewidth=2)
...
y_values_to_consider = [
    ...
    components['noaa'],
    components['noaa_slr'],
    components['station_SLP_prediction'],
...
ax.plot(plot_data['datetime'], plot_data['station_SLP_prediction'], color='g', label="NOAA + SLR + SLP", linewidth=0.8)
...
y_values_to_consider = [
    ...
    plot_data['noaa'],
    plot_data['noaa_slr'],
    plot_data['station_SLP_prediction'],
